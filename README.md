...
Đề tài tập trung giải quyết **bài toán ngược (Inverse Problem)** trong hệ thống định vị y tế không dây: Dịch ngược tín hiệu điện áp cảm ứng (EMF) thu được từ phần cứng thành tọa độ không gian 3D thực tế (X, Y, Z) và góc quay định hướng (Roll, Pitch, Yaw) của viên nang nội soi.
...
* **Hệ thống Phát (TX):** 3 cuộn dây phát từ trường độc lập ($N_{tx} = 300$ vòng, bán kính $R = 6\text{ cm}$), kích thích ở các tần số khác nhau: 5050 Hz, 6910 Hz, 5930 Hz.
* **Hệ thống Thu (RX):** Cảm biến vi sai gồm 2 nửa cuộn dây cách nhau khoảng vi sai $d = 0.25\text{ mm}$.
  * *Đặc thù phần cứng:* Cảm biến RX2 được bố trí nằm ngang hướng thẳng lên trục Z, trong khi RX3 hướng theo trục Y và RX1 theo trục X.
* **Dữ liệu thực nghiệm (`conenorot_data.csv`):** Chứa tín hiệu điện áp đo đạc thô ở 9 kênh (3 TX x 3 RX) kết hợp với dữ liệu chuẩn Ground Truth từ thiết bị cơ khí.
...
* **Nhiệm vụ:** Khai báo toàn bộ các hằng số vật lý cốt lõi của phần cứng hệ thống (số vòng dây phát $N_{tx}$, bán kính cuộn $R$, tần số kích thích $f$, dòng điện kích thích $I$, khoảng cách vi sai $d = 0.25\text{ mm}$, diện tích mặt cắt $AK$). Đặc biệt là nạp ma trận tọa độ và góc đặt nghiêng thực tế của 3 cuộn phát TX.
...
* **Nhiệm vụ:** Đọc file thực nghiệm `conenorot_data.csv`. Trích xuất dữ liệu thành 3 ma trận độc lập: 9 cột tín hiệu điện áp đo thực tế (EMF), 3 cột tọa độ thực tế (`GT_pos`), và 3 cột góc quay (`GT_ang`). Code tích hợp cơ chế kiểm tra lỗi tệp và **tự động quy đổi đơn vị tọa độ** từ mm sang mét (m) chuẩn SI.
...
* **Nhiệm vụ:** Xây dựng hàm toán học mô phỏng quá trình sinh từ trường của lưỡng cực từ. Thay vì dùng xấp xỉ đạo hàm vi phân, mô hình áp dụng phương pháp Rời rạc (Discrete): Tính toán cảm ứng từ trực tiếp tại hai nửa cuộn thu cách nhau khoảng $d$, sau đó lấy hiệu số $\Delta B$. Thuật toán ánh xạ chính xác thiết kế phần cứng (RX2 theo trục Z, RX3 theo trục Y, RX1 theo trục X).
...
* **Nhiệm vụ:** Tính toán sai số lệch chuẩn toàn cục (RMSE) tính bằng milimet. Xuất 2 bộ đồ thị phân tích: Đồ thị chồng chập quỹ đạo 3D (Thực tế vs Ước lượng) và Lưới đồ thị phân tích tín hiệu 3x3.
* **Ý nghĩa:** Lưới 3x3 so sánh trực tiếp đường cong điện áp Đo đạc (Đen) và Lý thuyết (Đỏ), cung cấp bằng chứng khoa học tuyệt đối về mức độ hội tụ của hệ thống mô hình.
